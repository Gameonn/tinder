...
if(!($token && $profile_pic)){
	...
	$image=str_replace('http://code-brew.com/projects/naseeb/uploads/','',$profile_pic);
	...
	$sql="UPDATE gallery set image='{$image}' WHERE id='{$old_image_id}' and user_id='{$uid}'";
	...
	$sql="UPDATE gallery set image='{$gallery_pic}' WHERE id='{$new_image_id}' and user_id='{$uid}'";
	$sth=$conn->prepare($sql);
	//$sth->bindValue('new_image',$image);
	//$sth->bindValue("old_image",$gallery_pic);
	//$sth->bindValue("user_id",$uid);
