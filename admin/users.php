...
global $conn;
$sql="SELECT id,username,gender,profile_pic,age,city,(SELECT count(user_like.id) FROM user_like WHERE user_like.liked_by=users.id and user_like.status=1 
and liked_to IN (select liked_by from user_like where liked_to=users.id and user_like.status=1)) as match_count FROM `users`";
...
global $conn;
$sql="SELECT group_concat(users.username SEPARATOR ', ') as uname
FROM `user_like`  
...
<div class="row">
			...
			<button type="submit" onclick="getCSVData()" class="btn btn-primary" ><i class="fa fa-bars"></i> Export Users</button>
                    ...
                    <tr class="gradeX">
                        ...
                        <td class="center hidden-phone"><?php echo $value['age']; ?></td>
                        <td class="center hidden-phone"><?php echo $value['city']?$value['city']:'-'; ?></td>
                        <td class="center hidden-phone"><?php echo getMatchedUsers($value['id']); ?></td>
                    </tr>
					<?php } ?>
                  <!--  <tfoot>
                    <tr>
